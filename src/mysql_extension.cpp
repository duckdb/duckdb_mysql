...
//	config.AddExtensionOption("pg_pages_per_task", "The amount of pages per task", LogicalType::UBIGINT, Value::UBIGINT(MySQLBindData::DEFAULT_PAGES_PER_TASK));
//	config.AddExtensionOption("pg_connection_limit", "The maximum amount of concurrent MySQL connections", LogicalType::UBIGINT, Value::UBIGINT(MySQLConnectionPool::DEFAULT_MAX_CONNECTIONS), SetMySQLConnectionLimit);
//	config.AddExtensionOption("pg_array_as_varchar", "Read MySQL arrays as varchar - enables reading mixed dimensional arrays", LogicalType::BOOLEAN, Value::BOOLEAN(false));
//	config.AddExtensionOption("pg_experimental_filter_pushdown", "Whether or not to use filter pushdown (currently experimental)", LogicalType::BOOLEAN, Value::BOOLEAN(false));
	config.AddExtensionOption("mysql_debug_show_queries", "DEBUG SETTING: print all queries sent to MySQL to stdout", LogicalType::BOOLEAN, Value::BOOLEAN(false), SetMySQLDebugQueryPrint);
